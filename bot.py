...
class TwitchBot(irc.bot.SingleServerIRCBot):
    def __init__(self, debug):
        ...
        self.commands += self.guessing_game.commands['normal']
    ...
    def get_channel_id(self):
        self.channel_id = twitch.get_user_id(self.channel_name)
        self.logger.debug('Channel ID is %s', self.channel_id)

    def get_self_id(self):
        self.id = twitch.get_user_id(self.username)
    ...
    def irc_connect(self):
        ...
        irc.bot.SingleServerIRCBot.__init__(
            self, [(server, port, twitch.TOKEN)], self.username, self.username)
